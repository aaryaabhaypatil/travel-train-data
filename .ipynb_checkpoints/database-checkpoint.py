...
def list_opalcards_equifilter(attributename, filterval):
    # Get the database connection and set up the cursor
    conn = database_connect()
    if(conn is None):
        # If a connection cannot be established, send an Null object
...
def add_opalcard_insert(firstname, lastname, userroleid, password):
    """
    Add a new User to the system
    """
    # Data validation checks are assumed to have been done in route processing
    ...
    cur = conn.cursor()
    sql = """
        INSERT into opaltravel.OpalCards(firstname, lastname, userroleid, password)
        VALUES (%s,%s,%s,%s);
        """
    print_sql_string(sql, (firstname, lastname, userroleid, password))
    try:
...
def update_single_card(userid, firstname, lastname, userroleid, password):
    # Get the database connection and set up the cursor
    conn = database_connect()
    if(conn is None):
        # If a connection cannot be established, send an Null object
...
def delete_opalcard(userid):
    ...
    try:
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error deleting the card from the OpalCards database ",userid, sys.exc_info()[0])
        cur.close()                     # Close the cursor
        conn.close()                    # Close the connection to the db
        raise
